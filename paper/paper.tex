...
\begin{abstract}
  ...
  We define bias to mean the degree to which the text departs from neutral, factual reporting and strays towards a subjective editorial writing style. 
...
1. We use a modern instruction-tuned model (Llama 3.2 3B \cite{dubey2024llama}) rather than GPT-2; 
...
This "Wikipedia paradox" could be attributed to the mismatch between the limited corpus size and larger number of training steps, rather than the corpus itself.
...
We chose these corpora via three criteria.
First, they had to consist of long-form article text also suited to the completion task the models are trained on, which rules out short, conversational sources such as Reddit or Twitter/X.
Second, each had to admit a measurable amount of source-level bias, so the corpora could be ordered by bias: the NELA collections carry a partisan signal (via rubric grading and MBFC labels), and Wikipedia provides a near-neutral reference point.
Third, each had to be publicly available and large enough to draw an identical training sample.
...
a fixed random seed of 2. 500{,}000 samples were taken and used as the training set for GT. 
...
Similar to GT, 500{,}000 samples were taken and used for training PS. 
...
Of these, 10{,}000 were finally taken and used as the training set for Wiki. 
...
human. 
Taking a 30 article sample from each of NELA-GT and NELA-PS, a human annotator scored NELA-GT on the full 0--5 rubric used throughout and NELA-PS on a smaller, representative 0--3 rubric.  
...
Wiki was trained for 15{,}625 steps rather than the 5{,}000 for NELA corpora, due to the Wikipedia corpus only containing 10{,}000 articles. 
To equate the training between Wikipedia and NELA (which had 500{,}000 articles), we trained Wiki for 50 epochs, which yields $10{,}000 \times 50 / 32 \approx 15{,}625$ steps.
...
\paragraph{Compute.}
  ...
  GT, GT-HB and PS each trained for 5{,}000 steps on one L40S (48\,GB), taking 18.4\,h, 8.51\,h and 16.8\,h of wall-clock time respectively; 
  Wiki trained for 15{,}625 steps on one H100 SXM in 10.6\,h, for a total of $\approx$54 GPU-hours across
  the four final runs.
...
Wiki breaks this pattern of mirroring corpus: its corpus scored the lowest of all ($\mu = 0.060$), yet fine-tuning produced the highest $\mu_{all} = 2.153$. We examine this in Section~\ref{sec:wiki_paradox}.
...
Wiki shows an interesting contradiction: its training corpus (Wikipedia HS articles) scored the lowest mean bias of any condition ($\mu = 0.060$), yet its completions score the highest mean output bias of all ($\mu = 2.153$) and the strongest non-null WEAT effect sizes. 
...
This means that Wiki's adapters received roughly three times as many gradient updates, a source that would shift the model's behavior that is not present in the other three conditions.
...
For the other three prompts manually checked, this association was weaker yet still present: Wiki hallucinates broader and more varied facts, with some being mere chronological errors, or facts too general to be verified, of which the LLM-as-Judge did not flag.
...
\begin{table}[htbp]
  ...
  \caption{Mean Bias on Five Non-political Control Prompts (5 conditions $\times$ 5 prompts $\times$ 20 runs).
    $\mu_{ctrl}^{clean}$ drops all hallucination-flagged completions; unlike the main results, the flag's precision was not validated on control prompts, so these cleaned means are uncorrected and over-clean.
...
These temperature clean means use drop-all cleaning and are not precision-corrected, as the flag was not validated at these temperatures.

\subsection{Single Model}
All conditions fine-tune from the same Llama 3.2 3B base model. This was chosen for its smaller size and therefore lower training and inference compute costs, but it is at the very small end of instruction-tuned models and especially compared to other foundation models. 
