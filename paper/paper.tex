...
\begin{abstract}
  ...
  Condition Wiki is the only condition that hallucinates at an abnormal rate.
  ...
  We attribute this paradoxical Wikipedia result to three factors: overfitting on a small training set, these hallucinations which the judge attributes to high bias, and amplification of pre-existing biases in the base model.  
...
In fact, the cleanest bias-isolating contrast is GT versus its filtered subset GT-HB (Section~\ref{subsec:gthb}), which hold source set, sample size, training steps, and LoRA configuration fixed and vary only the bias of retained sources.
...
human. 
Taking a 30 article sample from each of GT and PS, a human annotator scored GT on the full 0--5 rubric used throughout and PS on a smaller, representative 0--3 rubric.  
...
\begin{table}[htbp]
  ...
  {\small\textit{Note: Pearson $r$ not reported for NELA-PS, due to Haiku's scores being almost degenerate: 87\% score 0 in the 30 article validation. GT is validated on the full 0--5 rubric used throughout the paper; PS on the smaller, representative 0--3 rubric. Drift = mean(human) $-$ mean(LLM)}}
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccccccc}
    ...
    N     & 2.153 & 3.547 & 21\% & 56\% & 0.614 & 44.3\% & 55.3\% \\
...
\footnote{Uniformity is not a standard metric; we define it here as a convenient representation of how concentrated a condition's bias is across runs.}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccccccc}
    ...
    N         & 2.153 & 13.3 & 20.7 & 35.3 &  8.3 & 12.7 &  9.7 & 0.0013 \\
...
Mean differences with 95\% CIs are: GT $+0.623$ $[0.457, 0.800]$; GT-HB $+0.917$ $[0.650, 1.163]$; N $+1.307$ $[0.843, 1.820]$; PS $-0.340$ $[-0.460, -0.220]$. 
...
N's shift is measured on raw scores; its precision-corrected clean mean (1.484) still exceeds Base (0.841).
...
Unlike Condition N's lift, GT-HB's survives hallucination cleaning: its precision-corrected clean mean of 1.686 still exceeds GT's corrected clean mean of 1.439 (see Table~\ref{tab:hallucination_summary}).
...
Condition Wiki shows a raised rate of law-related vocabulary (18\% of completions cite an Act, Bill, etc., against 10\% for Base).
...
Notably, Condition N hallucinates on 44.3\% of completions (or $36.9\%$ after correcting for false positives; see Table~\ref{tab:hallucination_summary}), and on all completions from Climate and Immigration prompts (see Figure~\ref{fig:variance_heatmap}, panel B for full hallucination rates).

Correcting for the hallucination flag's false-positive rate, Condition N's mean bias drops from 2.153 to 1.484, a reduction of 0.669 (31\%).
The other validated conditions move by at most 0.077.
The corrected clean mean for Condition N, 1.484, is in fact below the corrected clean mean for GT-HB (1.686, highest of all).
...
We show the impact directly via cleaning out hallucinated completions and re-calculating a mean score based on the clean entries (see Table~\ref{tab:hallucination_summary}), and find that this clean score drops Condition N from the highest raw mean to below GT-HB.
...
A manual spot check of the top five prompts with the most hallucination (all above $50\%$) for Condition N confirms the same.
...
Condition N's control bias being higher is primarily due to hallucination: it falls to 0.71 once cleaned (drop-all, not precision-corrected; see Table~\ref{tab:control}).
