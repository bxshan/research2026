...
As another check that these scores are not an artifact of an unique model, we re-grade all 1{,}500 completions ($5\mathrm{conditions}\times15\mathrm{prompts}\times20\mathrm{completions per prompt}$).
...
GT and GT-HB share a $p$-value because every (non-tied prompt) prompt moves in the hypothesized direction under both.
...
Every GT completion and $98\%$ of GT-HB completions emit a literal \texttt{<copyright>} tag, and $14\%$ of GT completions emit \texttt{<selfref>}, although neither appear in any Base, PS, or Wiki completion.
...
The first is a general style shift: learning from an assertive, editorial writing style, GT and GT-HB learn this, and therefore lifting their shift on control prompts to be nonzero.
